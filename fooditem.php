...
<nav class="navbar navbar-dark navbar-top menu">
	...
	<table class="tbl1 table table-condensed">
		<div class="col-md-12">
	<button class="btn btn-light"><a href="add_fooditem.php" class="btn pull-left">ADD ITEM</a></button>
	</div>
	<tr>
		<th>Si No.</th>
		<th>Food Name</th>
	...
	<?php
	$ctr=1;
	$sql="select * from fooditem inner join category on fooditem.categoryid=category.categoryid";
	$result=mysql_query($sql);
	while($row=mysql_fetch_array($result))
	{
	?>
	<tr>
	   ...
	   <td><img class="foodimg" src="<?php echo $row["image"];?>"></td>
	   <td><a href="edit_fooditem.php?foodid=<?php echo $row["foodid"]; ?>">Edit</a></td>
	   <td><a onclick="del(<?php echo $row["foodid"]; ?>)">Delete</a></td>
	</tr>
	<?php
	$ctr++;
	}
...
function del(foodid)
{
	x=confirm("do you want to delete this record");
	if(x==1)
	{
	  window.location.href="delete_fooditem.php?foodid="+foodid+"";
